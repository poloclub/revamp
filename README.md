...
`CUDA_VISIBLE_DEVICES=0 python revamp.py attack.target=cat scenario.randomize_positions=true`

#### Specify Target Class and Use a Different Scene
`CUDA_VISIBLE_DEVICES=0 python revamp.py attack.target=cat attack.passes=1 scene=cube_r1`
...
2. run: `python revamp.py attack.target=truck attack.passes=1 attack.passes_names=[7]`
...
`CUDA_VISIBLE_DEVICES=0 python revamp.py attack.target=stop_sign scenario.sensor_positions.function=use_provided_cam_position attack.iters=100 attack.passes=10 attack.multi_pass_rendering=true attack.samples_per_pixel=64 attack.multi_pass_spp_divisor=8 scene=city`
